"""Validate and combine the independently extracted Jihaksa K7/K8 datasets.
...
DEFAULT_OUTPUT = ROOT / 'outputs/extraction/jihaksa-20260922'
...
def main():
    ...
    for level, grade, page_count in [('K7', 1, 294), ('K8', 2, 352)]:
        path = args.output / level / f'science_experiment_supplies_jihaksa_{level}.json'
        obj = json.loads(path.read_text(encoding='utf-8-sig'))
        data = obj['data']
        source = args.source_dir / f'textbook_jihaksa_{level}.pdf'
        ...
        for r in data:
            ...
            if missing:
                ...
                continue
            if not isinstance(rid, str) or not rid.startswith(f'jihaksa_{level.lower()}_'):
                errors.append(f'{rid}: incorrect book-specific ID')
            if r['source_row'] is not None or r['출판사'] != '지학사':
            ...
            s = r['source']
            if s.get('book_id') != f'jihaksa_{level}':
        ...
        books.append({
            'book_id': f'jihaksa_{level}', 'grade': grade,
    ...
    combined = {
        'metadata': {
            'schema_version': '1.0-jihaksa-extraction', 'record_count': len(rows),
            'source_books': books,
            'original_columns': ['단원명', '성취기준', '출판사', '쪽', '탐구활동', '교구'],
            'notes': [
                'K7 and K8 are independent book extractions; existing 454 activity rows are not merged.',
                ...
                'Source PDFs contain teacher guidance; activities target the student textbook region.',
    ...
    write_json(args.output / 'science_experiment_supplies_jihaksa.json', combined)
