...
(async () => {
  ...
  const expectedOrder = [...activities].sort((a,b) => a.achievement_raw.match(/^\d+-\d+/)[0].localeCompare(b.achievement_raw.match(/^\d+-\d+/)[0], 'en', {numeric:true})
  ...
  assert.equal(await page.locator('#dialog-content .dialog-section').nth(1).locator('p').innerText(), '미확인');
