...
(async () => {
  ...
  await page.waitForFunction(()=>document.getElementById('result-count').textContent==='탐구활동 1,276건');
  ...
  assert.equal(activities.some(activity => activity.id === 'donga_IS2_045'),false);
  ...
  assert.deepEqual([...new Set(await page.locator('.publisher-tag').allTextContents())].sort(),[...expectedPublishers].sort());
  ...
  await page.evaluate(key => localStorage.setItem(key, JSON.stringify({publishers:['비상'],publisherCatalog:['동아','미래앤','비상','천재(임성숙)','천재(정대홍)','지학사','YBM','동아출판','미래엔','비상교육','천재교과서']})),key);
  await page.reload();
  await page.waitForFunction(()=>document.getElementById('result-count').textContent==='탐구활동 209건');
  for(const publisher of publishers.filter(publisher => publisher !== '비상교육')) await page.locator(`input[data-publisher="${publisher}"]`).uncheck();
  assert.equal(await page.locator('#result-count').innerText(),'탐구활동 209건');
  assert.deepEqual(await page.locator('.activity-card').evaluateAll(cards=>cards.map(card=>card.dataset.activityId)),expectedOrder.filter(activity=>activity.publisher_raw==='비상교육').slice(0,30).map(activity=>activity.id));
  ...
  assert.deepEqual(stored.publishers,['비상교육']);
  assert.deepEqual(Object.keys(stored),['publishers','publisherCatalog']);
  assert.equal(Object.hasOwn(stored,'selected'),false);
  await page.reload();
  await page.waitForFunction(()=>document.getElementById('result-count').textContent==='탐구활동 209건');
  assert.equal(await page.locator('input[data-publisher="비상교육"]').isChecked(),true);
  await page.close();
  ...
  await page.waitForFunction(()=>document.getElementById('result-count').textContent==='탐구활동 209건');
  await page.locator('#reset-filters').click();
  // Canonical labels reach details as well as cards; all three 천재 labels stay distinct.
  for (const publisher of expectedPublishers.filter(name => name !== '지학사' && name !== 'YBM')) {
    const sample = activities.find(activity => activity.publisher_raw === publisher);
    assert.ok(sample);
    await page.locator('#search').fill(sample.title);
    const card = page.locator(`[data-activity-id="${sample.id}"]`);
    assert.equal(await card.locator('.publisher-tag').innerText(),publisher);
    await card.locator('.card-actions button').click();
    assert.equal(await page.locator('#dialog-content dt').filter({hasText:/^출판사·저자$/}).locator('xpath=following-sibling::dd[1]').innerText(),publisher);
  ...
  await checkUnitOptions(activities,29);
  assert.equal((await page.locator('#unit-filter').innerText()).includes('부록'),false);
  await page.locator('#search').fill('직업 탐구 활동');
  ...
  await checkUnitOptions(highActivities,6);
  ...
  await blockedPage.waitForFunction(()=>document.getElementById('result-count').textContent==='탐구활동 1,276건');
  await blockedPage.locator('input[data-publisher="비상교육"]').uncheck();
  assert.equal(await blockedPage.locator('#storage-warning').isVisible(),true);
  assert.equal(await blockedPage.locator('#result-count').innerText(),'탐구활동 1,067건');
