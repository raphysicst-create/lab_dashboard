...
(async () => {
  ...
  await page.locator('#chemical-search-results').getByRole('button',{name:'염산',exact:true}).click();
  ...
  await page.getByRole('button',{name:/사용 활동 보기/}).click();
  assert.equal(await page.locator('#chemical-dialog').isVisible(),false);
  assert.ok((await page.locator('#active-chemical').innerText()).includes('에탄올'));
  ...
  const ethanol=chemicals.find(c=>c.name==='에탄올');
  assert.equal(await page.locator('#result-count').innerText(),`탐구활동 ${activities.filter(a=>a.chemical_ids.includes(ethanol.id)).length}건`);
  await page.locator('#active-chemical').getByRole('button',{name:'약품 선택 해제'}).click();
  ...
  await browser.close();
  const report={status:'passed',checked:['main search to management','source storage rules','activity to chemical modal and Escape','catalogue and normalized notation search','conflicting cabinet stays unknown','chemical activity filter','shared guidance','unmatched unknown','mobile layout'],pageErrors:errors};
