...
def main():
    ...
    for row in rows:
        ...
        grade = GRADE_BY_UNIT.get(unit_number)
        ...
        aid = key('ACH', json.dumps([unit, standard], ensure_ascii=False))
        ...
        activity = {
            ...
            # Do not split ambiguous source prose into invented individual materials.
            'material_ids': [],
        }
        ...
        review.append({'activity_id': row['id'], 'source_row': row['source_row'],
                       ...
                       'grade_mapping': 'user_specified' if grade is not None else 'not_checked',
                       'grade_mapping_basis': 'User instruction: units 1-8 = grade 1; units 9-15 = grade 2',
    ...
    write(DEST / 'sources.json', {'source_name': source['metadata']['source_file'],
          'source_sha256': source['metadata']['source_sha256'], 'record_count': len(rows)})
