...
for (const [i, row] of source.data.entries()) {
  ...
  const unitNumber = Number(row['단원명'].match(/^(\d+)\./)[1]);
  assert.equal(activity.unit_number, unitNumber);
  assert.equal(activity.grade, unitNumber <= 8 ? 1 : 2);
...
assert.deepEqual([...new Set(activities.map(a => a.unit_number))].sort((a, b) => a - b), Array.from({length: 15}, (_, i) => i + 1));
...
assert.equal(activities.filter(a => a.materials_raw == null).length, 25);
...
assert.equal(filterActivities(activities, defaults).length, 454);
...
assert.equal(filterActivities(activities, {...defaults,grade:'1'}).length, 178);
assert.equal(filterActivities(activities, {...defaults,grade:'2'}).length, 276);
